...
| **Privacy conflict** | The derived condition that a public steward repository holds a case whose complete privacy is private, because the case's recorded privacy is private or a current participant is private. Case queries report it as `privacy_conflicted`, or as unknown when no portfolio root selection resolves the participants. Current-participant conflict warns before a later event refuses. Recorded-private case privacy directly refuses a public-ward steward visibility transition; if the marker changes outside the CLI, queries still report the conflict. |
