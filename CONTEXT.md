...
| **Reuse decision** | The exact human-accepted disposition of a case, including its scope, non-responsibilities, verification conditions, and, when its action authorizes implementation, its migration expectations. |
...
| **Portfolio location** | Where a command resolves the portfolio from: the roots supplied for this run, the user-local configuration file that would otherwise name them, and the user-local state file holding the previous observation. It is resolved once per invocation from the supplied roots and the process environment, and records an undeterminable configuration or state directory as an absence rather than a failure, so each command decides whether that absence refuses or degrades. A location is not a root: a root is a directory that gets scanned. |
...
| **Steward repository** | The one repository that owns a case's authoritative event stream and accepted decision. Stewardship is bound to the opaque repository identity recorded in the case events, not to a directory: a repository that acquires a new identity no longer stewards cases opened under the previous one. Other participating repositories are referenced consumers, not synchronized copies. |
| **Identity substitution** | The condition where a recorded repository identity appears in no current enrollment while its recorded path is now enrolled under a different identity. The path survives; the identity that occupied it does not. Portfolio reports it as its own change class carrying both identities and both visibilities, before the previous identity leaves portfolio state. Deliberate un-enrollment, where no identity takes the path, and a move, where the identity survives elsewhere, are different conditions. |
...
| **Privacy conflict** | The derived condition that a public steward repository holds a case whose complete privacy is private, because the case's recorded privacy is private or a current participant is private. Case queries report it as `privacy_conflicted`: true when a conflict is established, unknown when none is established and either no portfolio root selection resolves the participants or a recorded participant does not resolve to exactly one enrolled repository, and false otherwise. An established conflict is never downgraded to unknown by an unresolvable participant. A private-to-public steward transition refuses before it can create the recorded-privacy condition; the current-participant condition warns before a later event refuses. The derived condition is not itself a refusal. |
