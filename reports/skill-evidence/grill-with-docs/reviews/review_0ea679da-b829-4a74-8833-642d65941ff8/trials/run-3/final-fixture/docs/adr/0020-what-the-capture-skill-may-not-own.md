...
Capture may render and present a prepared proposal as the exact bounded decision surface for human acceptance. This presentation does not make capture the owner of issue decomposition or implementation routing: both remain outside capture ownership and, after acceptance, implementation remains with the ordinary engineering workflow.

This amendment clarifies the existing boundary without changing the decision, its status, or its authorized command and placement rules.
