...
| [0023](0023-disclosure-boundary-for-reuse-review-package-research.md) | Reuse review may make a package-research network call; nothing compiled may. A call is one HTTP request, allowed only when five conditions checkable before it all hold, over a closed class of permitted hosts whose instance the human also approved. A private case's terms must be generalized, and the human approves the generalization. Declining makes no call, and without supplied findings bars recommending a new public crate. Findings are recorded on the decision event, not as an evidence reference. |
| [0024](0024-what-the-installer-ships-once-the-set-grows.md) | The shipped package set is defined by the reserved `reuse-evidence-` prefix rather than enumerated, and authoring a package there is the act of shipping it. A path under that prefix the running binary does not ship is a stale asset — a retired package directory or a file orphaned inside a shipped one — and it refuses the install rather than being removed. Atomic refusal spans the whole set, and the crate's named file set follows the same rule. Adds one adopter obligation to `CONSUMER-CONTRACT.md` §6. |
