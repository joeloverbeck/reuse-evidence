...
**Amended:** 2026-08-12 by explicit maintainer acceptance. Every event-receipt spine field has exactly one subject and keeps it. The heading, `file`, `revision`, and the event type's own notice report the event this command recorded or matched; the readiness fields and `privacy:` report the case as it now stands. An `Existing` outcome therefore reports the case's derived state for every later event type, rather than the state the retried event projected when it was prepared. The two subjects coincide on every path except an idempotent retry of a superseded event, which is why the 2026-08-10 amendment below could close the receipt question on evidence that did not cover it.
...
Amended 2026-08-12: that retraction was drawn from the preview and created paths, where a derived readiness and a stated constant necessarily coincide because the event is not yet part of the history the case was read from. On an idempotent retry the event is already in that history, and a retry of a *superseded* event makes the two diverge: a closed case retrying its early-review override reported `state: review-ready` and the review-only readiness notice, while a retry of the same case's superseded verification reported the live `state: closed` beside its own `parked` heading. The difference does reach the receipt; the 2026-08-10 evidence simply could not see it, and no test reached the path.
...
Amended 2026-08-12: one further receipt changes, again with no recorded evidence touched, because a retry writes nothing. An exact retry of a superseded early-review override or reuse decision reports the case's current state instead of a constant, so a retry against a case that has since moved on no longer prints a stale `state:`, `readiness_basis:`, or review-only readiness line. `case append` and `case verify` retries are unchanged, having always reported the live state. This too is a `CONSUMER-CONTRACT.md` §8 command-surface change during `0.x`. It completes rather than weakens `FOUNDATIONS.md` §11: a retry that is idempotent but reports a state the case does not hold satisfies the side-effect half of that principle and not the inspectable-receipt half.
...
| Report the state as of the retried event, so the whole receipt speaks for that event (2026-08-12) | Rejected | Defensible, and it is what `case override` and `case decide` shipped. But it is only cheap where a constant happens to equal the historical state. `append` derives readiness from the occurrence count, and `state_after_appending_occurrence()` counts against the *current* record, so a superseded retry would need a true replay of events up to that sequence. Nothing derives historical state that way, and `FOUNDATIONS.md` §11 asks for an inspectable receipt, not a reconstructed one. |
| Omit `state:` from the `Existing` outcome entirely (2026-08-12) | Rejected | It dissolves the question rather than answering it, and removes readiness from all four retry receipts to repair two. `case show` already answers where a case stands, but a retry receipt that reports privacy for the current case and nothing else about it is a narrower instrument than the resume path needs. |

## Verification and review trigger

The decision is fit if a third later event type is implemented by supplying its own proposal, eligibility, privacy, and receipt while reusing the publication protocol unchanged, and if the retry-privacy question has exactly one answer afterwards. The 2026-08-12 amendment adds the same test for the retry-state question, and requires that a superseded retry of every later event type be covered by a test, since the two answers that shipped were indistinguishable to a suite that only ever retried a case's latest event.

Reopen when a third later event type exists and its proposal shape, eligibility rule, and privacy timing match the existing two closely enough that a parameterization would couple nothing that changes for different reasons. At that point the wider collapse has three occurrences behind it and should be reconsidered on its merits. The receipt question is settled for the preview and created paths by the 2026-08-10 amendment and for the idempotent-retry path by the 2026-08-12 amendment, which gives each spine field one subject. Reopen it if a later event type needs a spine field whose subject is neither the event recorded or matched nor the case as it now stands.
